...
public final class DecisionAuditMermaidRenderer {
    ...
    public String analysis(AnalysisManifest.AnalysisResult analysis) {
        ...
        var diagram = new AuditDiagram(
                "Source constructs for " + selectedMethod(analysis),
                "Analysis decisions", "Exact result-relevant graph");
        for (AnalysisManifest.AnalysisDecision decision : analysis.manifest().analysisDecisions()) {
            String sourceKey = sourceKey(decision);
            diagram.source(sourceKey, sourceKey, sourceExample(decision));
    ...
    public String projection(BusinessGraphAudit audit) {
        ...
        var diagram = new AuditDiagram(
                "Exact graph inputs for " + audit.graph().decisionLabel(),
                "Projection decisions", "Final business graph");
    ...
    private static String selectedMethod(AnalysisManifest.AnalysisResult analysis) {
        return analysis.manifest().probeSites().stream()
                .filter(site -> site.kind() == AnalysisManifest.ProbeKind.ENTRY)
                .map(site -> site.ownerHint() + "#" + site.memberHint() + site.descriptorHint())
                .findFirst().orElse(analysis.graph().decisionLabel());
    }
