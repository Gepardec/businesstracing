...
The canvas is read-only. It opens with the **Readable** presentation in **Explore** mode. Readable groups safe action chains, safe rule chains, and parallel connections without changing the source JSON. **Full detail** restores every source node and edge. The summary above the canvas explains the entry, first material split, work, cycles, and terminal results in no more than three sentences.

Explore creates a separate compact layout for the entry path through its first material split. After selection or search, it shows the selected node with its direct predecessors and successors. Select a visible node to continue through the graph. Use **Overview** to fit the complete current presentation. Node kind, business-label text, and route labels stay present while you zoom. At a small zoom, point at a node to show its full-size kind and label. Graphs with more than 100 nodes use search guidance instead of a minimap.
