...
Every predicate branch shows its outcome on the route near the source. Boolean values display as `Yes` and `No`; the accessible edge name keeps the raw value. Route labels stay present while you zoom. Repeated kind-and-label pairs show an occurrence marker, such as `2 of 3`, and keep their exact node IDs in the accessible description.

Four or more routes that enter one node use one presentation-only junction and shared trunk. Each supplied edge stays a separate keyboard focus target. Pointer or keyboard inspection highlights its feeder and the shared trunk. Long return or reference connections use a light solid line until inspection, so they do not look like the normal top-to-bottom decision flow. Cycle returns use a safe perimeter corridor. Route crossings use a bridge, not a graph node. Cycles and disconnected components use tinted structural outlines. These presentation items do not change the supplied node or edge count.

The canvas is read-only. It opens with the **Readable** presentation in **Explore** mode. Readable groups safe action chains, safe rule chains, and parallel connections without changing the source JSON. **Full detail** restores every source node and edge. The explanation panel states the entry, first material alternatives, and terminal results in no more than three sentences. It lists every member of a selected sequence and every immediate continuation. A continuation with no supplied outcome uses its target business label in the panel and has no invented route label.

Explore creates a separate compact layout for the entry path through its first material split. After selection or search, it shows the selected node with its direct predecessors and successors. Select a visible node or a continuation in the explanation panel to move through the graph. Use **Overview** as a topology map of the complete current presentation. Node kind, business-label text, and supplied route labels stay present while you zoom. At a small zoom, point at a node to show its full-size kind and label in the explanation panel. Graphs with more than 100 nodes use search guidance instead of a minimap.
