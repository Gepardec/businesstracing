...
export interface GraphModel {
  schema: typeof DEVELOPER_GRAPH_SCHEMA;
  id: string;
  version: number;
  label: string;
  entryNodeId: string;
...
function exactKeys(value: JsonObject, allowed: readonly string[], path: string): void {
  ...
  if (unknown) throw new ContractError(`${path} has an unknown field: ${unknown}`);
}

export function parseDeveloperGraph(input: unknown): GraphModel {
  const root = object(input, 'graph document');
  exactKeys(root, ['schema', 'graph', 'sourceOrigins', 'sourceFiles'], 'graph document');
  ...
  exactKeys(graph, ['id', 'version', 'label', 'entryNodeId', 'completeness', 'nodes', 'edges', 'coverageGaps'], 'graph');
  const origins = array(root.sourceOrigins, 'sourceOrigins').map((raw, index) => {
    const origin = object(raw, `sourceOrigins[${index}]`);
    exactKeys(origin, ['id', 'kind', 'identity', 'checksum', 'revision'], `sourceOrigins[${index}]`);
  ...
  for (const [index, raw] of array(root.sourceFiles, 'sourceFiles').entries()) {
    ...
    exactKeys(file, ['originId', 'path', 'sha256'], `sourceFiles[${index}]`);
  ...
  const nodes = array(graph.nodes, 'graph.nodes').map((raw, index): GraphNode => {
    ...
    exactKeys(node, ['id', 'kind', 'label', 'attributes', 'source'], `graph.nodes[${index}]`);
    if (node.source !== undefined) {
      const source = object(node.source, `graph.nodes[${index}].source`);
      exactKeys(source, ['originId', 'path', 'line', 'column', 'syntaxKind', 'sha256', 'url'], `graph.nodes[${index}].source`);
  ...
  const edges = array(graph.edges, 'graph.edges').map((raw, index): GraphEdge => {
    ...
    exactKeys(edge, ['id', 'from', 'to', 'outcome'], `graph.edges[${index}]`);
    return Object.freeze({
  ...
  const coverageGaps = array(graph.coverageGaps ?? [], 'graph.coverageGaps').map((raw, index): CoverageGap => {
    ...
    exactKeys(gap, ['nodeId', 'description'], `graph.coverageGaps[${index}]`);
    return Object.freeze({
