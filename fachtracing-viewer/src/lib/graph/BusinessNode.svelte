...
<style>
  .business-node { width: 232px; min-height: 92px; border: 1px solid var(--border); border-left: 5px solid var(--node-color); border-radius: 12px; background: var(--card); box-shadow: 0 4px 18px color-mix(in oklch, var(--foreground), transparent 91%); overflow: hidden; transition: opacity 120ms, box-shadow 120ms; }
  ...
  .node-step { margin-left: auto; width: 23px; height: 23px; display: grid; place-items: center; border-radius: 999px; background: var(--run-current); color: white; font-size: 11px; font-weight: 800; }
  ...
  .business-node--entry { --node-color: var(--node-entry); border-color: var(--node-color); border-radius: 999px; }
  ...
  .business-node--outcome { --node-color: var(--node-outcome); border-color: var(--node-color); box-shadow: inset 0 0 0 3px var(--card), inset 0 0 0 4px var(--node-color), 0 4px 18px color-mix(in oklch, var(--foreground), transparent 91%); }
  ...
  .node-path:not(.node-current) { box-shadow: inset 0 0 0 2px var(--run-path), 0 4px 18px color-mix(in oklch, var(--run-path), transparent 82%); }
  .node-current { outline: 3px solid var(--run-current); outline-offset: 3px; box-shadow: 0 8px 26px color-mix(in oklch, var(--run-current), transparent 76%); }
