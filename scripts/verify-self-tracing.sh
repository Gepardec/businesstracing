...
grep -F -q 'choose by node kind' "$NODE_BASE-business.mmd"
...
grep -F -q 'does not have graph entry source' "$SOURCE_BASE-business.mmd"
...
rm -f "$RUNTIME_OUTPUT"/*.mmd 2>/dev/null || true
