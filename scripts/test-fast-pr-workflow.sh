...
require_text "jakartaee-rest:" "$WORKFLOW"
require_text "viewer:" "$WORKFLOW"
...
require_text "key: jakartaee-rest-\${{ runner.os }}-85da1d6861fea14579b1c6eb76253f0549a8e80f" "$WORKFLOW"
...
require_text "run: ./scripts/verify-jakartaee-rest.sh" "$WORKFLOW"
require_text "run: ./scripts/verify-postgres.sh" "$WORKFLOW"
require_text "run: ./scripts/verify-viewer.sh" "$WORKFLOW"
require_text "run: ./scripts/verify-viewer-dogfood.sh" "$WORKFLOW"
require_text "run: npm ci --ignore-scripts" "$WORKFLOW"
require_text "run: npm run build" "$WORKFLOW"
require_text "run: npm run test:browser" "$WORKFLOW"
require_text "uses: actions/upload-artifact@v6" "$WORKFLOW"
...
require_text "./scripts/verify-jakartaee-rest.sh" "$FAST"
