...
The corpus is selected before analyzer remediation and supplies no extraction hints. The annotation
overlay only marks entry methods and gives them business-facing titles.

| Business area | Annotated method | Why selected |
| --- | --- | --- |
...
calculator. A test-only method annotation is valid for the Java agent and avoids changing the
business implementation. Runtime invocation must occur through the public calculator path.
