...
| Exact external method contract | An explicitly enabled provider describes one exact binary owner, method name, and JVM descriptor. The analyzer uses source first, then one matching contract, then an explicit opaque-library boundary, and finally a coverage gap. A contract can prove a predicate, action, receiver or argument mutation, result behavior, possible exception types, and explicit incomplete-boundary facts. The analyzer keeps the proven operation and adds each declared gap. Two matches create a source-located conflict gap; the analyzer does not use provider priority |
...
- `jakarta-ee-soap-grpc-contracts`: The optional Jakarta EE adapter provides exact contracts for CDI-adjacent services, JPA, JTA, Bean Validation, EJB, Jakarta Security, JAX-RS and REST Client, JMS, JSON-B, Mail, Servlet, WebSocket, SOAP setup, and gRPC channel setup. Contracts with callbacks, infrastructure, or remote peers retain the known local operation and add an explicit gap.
...
- Cancellation of supported `Future`, `CompletableFuture`, and `ForkJoinTask` results releases the
  reservation exactly once and keeps the original result object unchanged. Activation scans compiled
  application output for the exact supported `cancel(boolean)` calls. It fingerprints those caller
  classes even when they have no graph probe. It does not scan dependency directories or JAR files.
