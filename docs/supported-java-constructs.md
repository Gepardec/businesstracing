...
- `source-unavailable-call`, `jakarta-platform-operation`, `external-archive-reference-operation`
...
| External archive reference operation | Treats a reference-returning operation from the first matching dependency JAR as an opaque library boundary. An instance operation keeps its receiver in the result slice, so source-visible predicates that configure fluent query or options objects remain visible. A Boolean archive call is transparent only inside an explicit source control condition, where the call site is already the graph predicate. Other primitive results and application class directories stay outside this boundary |
...
- Unsupported application decision logic is never reported as complete. Reference-returning dependency-JAR internals are outside the application decision boundary; source-visible application predicates around those calls remain in the graph.
